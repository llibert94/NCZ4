...
{
    Minkowski mink;
    KerrSchild<> kerr_schild(m_p.bg_params, m_dx, mink); // background

    // Enforce positive chi and alpha
    BoxLoops::loop(PositiveChiAndAlpha<KerrSchild<>>(m_dx, m_p.center,
                                                     kerr_schild, m_p.min_chi,
                                                     m_p.min_lapse),
...
void KerrBHLevel::initialData()
{
    ...
    BoxLoops::loop(make_compute_pack(SetValue(0.),
                                     KerrBH<KerrSchild<>>(m_p.kerr_params, m_dx,
                                                          kerr_schild)),
                   m_state_new, m_state_new, INCLUDE_GHOST_CELLS);

    fillAllGhosts();
    BoxLoops::loop(GammaCalculator<KerrSchild<>>(m_dx, m_p.center, kerr_schild),
                   m_state_new, m_state_new, EXCLUDE_GHOST_CELLS);

#ifdef USE_AHFINDER
    // Diagnostics needed for AHFinder
    Constraints<KerrSchild<>> my_constraints(m_dx, m_p.center, kerr_schild,
                                             c_Ham, Interval(c_Mom1, c_Mom3));
    ConformalDiagnostics<KerrSchild<>> my_diagnostics(
        m_dx, m_p.center, kerr_schild, Interval(c_g11, c_g33), c_chi);
                                  ...
                                  const double a_time)
{
    ...
    if (m_p.max_spatial_derivative_order == 4)
    {
        BoxLoops::loop(
            GHCRHS<MovingPunctureGauge<KerrSchild<>>, FourthOrderDerivatives,
                   KerrSchild<>>(m_p.ghc_params, m_dx, m_p.sigma, m_p.center,
                                 kerr_schild),
    ...
    {
        BoxLoops::loop(
            GHCRHS<MovingPunctureGauge<KerrSchild<>>, SixthOrderDerivatives,
                   KerrSchild<>>(m_p.ghc_params, m_dx, m_p.sigma, m_p.center,
                                 kerr_schild),
            a_soln, a_rhs, EXCLUDE_GHOST_CELLS);
    }
...
void KerrBHLevel::specificPostTimeStep()
{
    ...
    if (m_p.activate_extraction == 1)
    {
        ...
        if (calculate_adm)
        {
            ...
            fillAllGhosts();
            BoxLoops::loop(
                ADMQuantities<KerrSchild<>>(m_p.extraction_params.center, m_dx,
                                            kerr_schild, c_Madm, c_Jadm),
                m_state_new, m_state_diagnostics, EXCLUDE_GHOST_CELLS);
    ...
    {
        fillAllGhosts();
        Constraints<KerrSchild<>> my_constraints(
            m_dx, m_p.center, kerr_schild, c_Ham, Interval(c_Mom1, c_Mom3));
        ConformalDiagnostics<KerrSchild<>> my_diagnostics(
            m_dx, m_p.center, kerr_schild, Interval(c_g11, c_g33), c_chi);
        BoxLoops::loop(make_compute_pack(my_constraints, my_diagnostics),
                       m_state_new, m_state_diagnostics, EXCLUDE_GHOST_CELLS);
