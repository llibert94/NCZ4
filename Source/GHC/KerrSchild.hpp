...
template <class background_t = Minkowski> class KerrSchild
{
  public:
    ...
    KerrSchild(params_t a_params, double a_dx, background_t a_background)
                          ...
                          const Coordinates<data_t> &coords) const
    {
        ...
        FOR(i, j)
        {
            g[i][j] = TensorAlgebra::delta(i, j) + 2.0 * H * el[i] * el[j];
        }
                        ...
                        const Coordinates<data_t> &coords) const
    {
        ...
        FOR(i, j)
        {
            vars.h[i][j] = TensorAlgebra::delta(i, j) + 2.0 * H * el[i] * el[j];
        }
        ...
        {
            vars.shift[i] = 0.;
            FOR(j) { vars.shift[i] += g_UU[i][j] * 2.0 * H * el[j] * el_t; }
        }
        ...
        FOR(i, j, k, l)
        {
            d2.h[i][j][k][l] =
                2.0 *
                (dldx[i][l] * el[j] * dHdx[k] + el[i] * dldx[j][l] * dHdx[k] +
        ...
        {
            d2.shift[i][j][k] =
                2.0 * dltdx[k] * dHdx[j] * pow(vars.lapse, 2.0) * el[i] +
  ...
  protected:
    ...
    void get_KS_derivs(Tensor<1, data_t> &dHdx, Tensor<2, data_t> &d2Hdx2,
                       ...
                       const data_t &H, const Coordinates<data_t> &coords) const
    {
                    ...
                    (drhodx[j] * drhodx[i] + rho * d2rhodx2[i][j] +
                     0.5 / sqrt(0.25 * (rho2 - a2) * (rho2 - a2) + a2 * z * z) *
                         (d2rhodx2[i][j] * rho * (rho2 - a2) +
                          ...
                          0.5 /
                              (0.25 * (rho2 - a2) * (rho2 - a2) + a2 * z * z) *
                              (rho * drhodx[j] * (rho2 - a2) +
                               delta(j, 2) * 2.0 * a2 * z) *
                              (rho * drhodx[i] * (rho2 - a2) +
                               delta(i, 2) * 2.0 * a2 * z)));
        }
        ...
        FOR(i, j)
        d2costhetadx2[i][j] = -(delta(j, 2) * drdx[i] + x[2] * d2rdx2[i][j] -
                                x[2] * 2.0 * r * drdx[i] * drdx[j] / r2) /
                                  r2 -
                              delta(i, 2) * drdx[j] / r2;
        ...
        {
            d2ldx2[0][i][j] =
                (delta(j, 0) * drdx[i] + x[0] * d2rdx2[i][j] +
                 drdx[j] * delta(i, 0) -
                 2.0 *
                     ((drdx[j] * drdx[i] + r * d2rdx2[i][j]) *
                          (r * x[0] + a * x[1]) +
                      r * drdx[i] *
                          (drdx[j] * x[0] + r * delta(j, 0) +
                           a * delta(j, 1))) /
                     (r2 + a2) -
                 2.0 * r * drdx[j] *
                     (x[0] * drdx[i] + r * delta(i, 0) + a * delta(i, 1) -
                      4.0 * r * drdx[i] * (r * x[0] + a * x[1]) / (r2 + a2)) /
                     (r2 + a2)) /
                (r2 + a2);
            d2ldx2[1][i][j] =
                (delta(j, 1) * drdx[i] + x[1] * d2rdx2[i][j] +
                 drdx[j] * delta(i, 1) -
                 2.0 *
                     ((drdx[j] * drdx[i] + r * d2rdx2[i][j]) *
                          (r * x[1] - a * x[0]) +
                      r * drdx[i] *
                          (drdx[j] * x[1] + r * delta(j, 1) -
                           a * delta(j, 0))) /
                     (r2 + a2) -
                 2.0 * r * drdx[j] *
                     (x[1] * drdx[i] + r * delta(i, 1) - a * delta(i, 0) -
                      4. * r * drdx[i] * (r * x[1] - a * x[0]) / (r2 + a2)) /
                     (r2 + a2)) /
                (r2 + a2);
            d2ldx2[2][i][j] = -(delta(j, 2) * drdx[i] + x[2] * d2rdx2[i][j] -
                                2.0 * r * drdx[j] * x[2] * drdx[i] / r2) /
                                  r2 -
                              delta(i, 2) * drdx[j] / r2;
        }
