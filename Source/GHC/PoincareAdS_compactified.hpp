...
class PoincareAdS
{
  public:
	...
	data_t z_reg = simd_max(1e-4, x[2]);
        FOR(i, j)
        {
            g[i][j] = delta(i, j) + delta(i, 2) * delta(j, 2) * 
		      (pow((1. + x[2]) / (z_reg * z_reg), 2.) - 1.);
        }

        FOR(i, j, k)
        {
            dg[i][j][k] = -4. * delta(i, 2) * delta(j, 2) * delta(k, 2) *
                            (1. + z * z) / pow(z_reg, 5.);
        }
    }
	...
	// populate ADM vars
	data_t den_lapse = simd_max(1. - z*z, 1e-4);
        vars.lapse = L * z / den_lapse;
	data_t z_reg = simd_max(1e-4, x[2]);
        FOR(i, j)
        {
            vars.h[i][j] = delta(i, j) + delta(i, 2) * delta(j, 2) * 
		    	   (pow((1. + x[2]) / (z_reg * z_reg), 2.) - 1.);
        }
        ...
        FOR(i, j, k)
        {
            d1.h[i][j][k] = -4. * delta(i, 2) * delta(j, 2) * delta(k, 2) * 
		    	    (1. + z * z) / pow(z_reg, 5.);
        }

        FOR(i, j, k, l)
        {
            d2.h[i][j][k][l] = 4. * delta(i, 2) * delta(j, 2) * delta(k, 2) * delta(l, 2) *
		    	       (5. + 3. * z * z) / pow(z_reg, 6.);
        }
        ...
        FOR(i)
        {
            d1.lapse[i] = delta(i, 2) * L * (1. + z * z) / (den_lapse * den_lapse);
        }

        FOR(i, j)
        {
            d2.lapse[i][j] = delta(i, 2) * d1.lapse[j] * 2. * z / den_lapse;
        }
	...
	data_t lapse_reg = simd_max(1e-4, vars.lapse);
