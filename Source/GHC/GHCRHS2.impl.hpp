    ...
    const vars_t<data_t> &advec) const
{
    ...
    auto ricci = GHCGeometry::compute_ricci_Z(vars, d1, d2, g_UU, chris, Z);
    ...
    diffCoeff = pow(sqrt(2. * diffCoeff / ((double)GR_SPACEDIM) *
    ((double)GR_SPACEDIM - 1)), m_params.lapidusPower) * pow(m_deriv.m_dx,
    m_params.lapidusPower - 1); diffCoeff = 0.5 * m_params.lapidusCoeff *
    pow(m_deriv.m_dx, 2) * diffCoeff; double m_dt = 0.25; data_t diffCoeffSafe =
    simd_min(diffCoeff, m_params.diffCFLFact * pow(m_deriv.m_dx, 2) / m_dt);
    //!< Do not allow the diffusion coefficient to
                                               //!< violate the Courant
    condition
    ...
    //diffCoeffSafe = diffCoeffSafe * (1.0 - pow(1.1,
    -m_params.diffCutoff/vars.lapse));

    Tensor<2, data_t> space_laplace_g = {0.};
    data_t sgn;
    FOR(i, j) {
        FOR(k) { space_laplace_g[i][j] += d2.g[i][j][k][k]; }
        //sgn = simd_conditional(simd_compare_gt(space_laplace_g[i][j],
    0.0), 1.0, -1.0);
        //space_laplace_g[i][j] = sgn * simd_min(diffCoeffSafe *
    abs(space_laplace_g[i][j]),
        //			100 *  abs(rhs.g[i][j]));
    ...
    FOR(i, j)
    {
        rhs.K[i][j] = advec.K[i][j] - covd2lapse[i][j] +
                      vars.lapse * ricci.LL[i][j] +
                      vars.K[i][j] * vars.lapse * (tr_K - 2. * vars.Theta) -
                      kappa1_times_lapse * (1. + m_params.kappa2) * vars.Theta *
                          vars.g[i][j];
        FOR(k)
        {
            rhs.K[i][j] +=
    ...
    FOR(i)
    {
        ...
        {
            rhs.Gam[i] +=
                g_UU[i][j] * (2. * vars.lapse * d1.Theta[j] +
                              (tr_K - 2. * vars.Theta) * d1.lapse[j]) -
