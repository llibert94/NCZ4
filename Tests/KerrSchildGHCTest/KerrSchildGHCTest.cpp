    ...
    for (int ires = 0; ires < num_resolutions; ires++)
    {
        ...
        if (ires == 0 && debug_plots_on)
        {
            ...
            for (bit.begin(); bit.ok(); ++bit)
            {
                ...
                IntVect iv = bit();
                if (iv[0] == N_GRID / 4 + N_GRID / 2 &&
                    iv[1] == N_GRID / 4 + N_GRID / 2 && iv[2] == 0 + N_GRID / 2)
                {
                    ...
                    double out2 = fixedbg_rhs_fab(iv, c_h11);
        ...
        /*       for (int i = c_g11; i <= c_B3; ++i)
           ...
           = "
                   ...
                   // save the L1 norm for the convergence check - gives sum of
           abs
                   // values
                   error_norms[ires][i] =
                       rhs_fab.norm(L1_norm, i, num_comps) * pow(N_GRID, -3);
